...
function kindFromErrorCode(errorCode: string | null): ContinuityIssueKind | null {
  switch (errorCode) {
...
export function buildDataContinuityReport(params: {
  ...
  const linkRows = sortRows(
    Array.from(byLinkCode.entries()).map(([linkCode, c]) => ({ linkCode, ...c })),
  ).slice(0, topLimit);

  const requestRows = sortRows(
    Array.from(byRequestId.entries()).map(([requestId, c]) => ({ requestId, ...c })),
  ).slice(0, topLimit);
